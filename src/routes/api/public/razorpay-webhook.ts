      ...
      POST: async ({ request }) => {
        ...
        if (evt.event === "payment.captured" && orderId) {
          const { data: sub } = await supabaseAdmin
            .from("subscriptions").select("id, tier").eq("razorpay_order_id", orderId).maybeSingle();
          if (sub) {
        ...
        if (evt.event === "payment.failed" && orderId) {
          await supabaseAdmin.from("subscriptions").update({ status: "failed" }).eq("razorpay_order_id", orderId);
        }
