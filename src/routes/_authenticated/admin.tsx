...
function Admin() {
  ...
  const { data: reports } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("reports")
        .select("id, reason, status, created_at, reported_profile, reporter_id")
  ...
  const { data: subs } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("subscriptions")
        .select("id, user_id, tier, status, amount_inr, expires_at, created_at")
        .order("created_at", { ascending: false })
        .limit(50);
      return data ?? [];
    },
  ...
  const { data: verifs } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("verifications")
        .select("id, profile_id, type, status, created_at")
        .eq("status", "pending")
        .order("created_at", { ascending: false });
      return data ?? [];
    },
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10 space-y-10">
      ...
      <Section title={`Recent reports (${reports?.length ?? 0})`}>
        <Table
          headers={["Reported", "Reporter", "Reason", "Status", "When"]}
          rows={(reports ?? []).map((r) => [
            r.reported_profile?.slice(0, 8),
      ...
      <Section title="Subscriptions">
        <Table
          headers={["User", "Tier", "Status", "Amount", "Expires"]}
          rows={(subs ?? []).map((s) => [
            s.user_id.slice(0, 8),
            s.tier,
            s.status,
