...
function InterestsPage() {
  ...
  const load = async () => {
    ...
    const otherIds = Array.from(
      new Set(list.map((r) => (r.from_profile === u.user!.id ? r.to_profile : r.from_profile))),
    );
    const { data: profiles } = otherIds.length
      ? await supabase
          .from("profiles")
          .select("id, display_name, city, state, sub_sect, is_verified")
          .in("id", otherIds)
      : { data: [] as ProfileLite[] };
    ...
    setRows(
      list.map((r) => ({
        ...r,
        other: byId.get(r.from_profile === u.user!.id ? r.to_profile : r.from_profile) ?? null,
      })),
    );
  };
  useEffect(() => {
      ...
      <div className="flex items-end justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            When both sides accept, a private chat opens.
          </p>
        </div>
      </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <Link
                    to="/profile/$id"
                    params={{ id: r.other?.id ?? "" }}
                    className="font-medium hover:underline truncate"
                  >
                    {r.other?.display_name ?? "Member"}
                  </Link>
                  {r.other?.is_verified && (
                    <Badge variant="secondary" className="h-5 px-1.5">
                      <ShieldCheck className="w-3 h-3 mr-0.5 text-primary" />
                    </Badge>
                  )}
                ...
                <div className="text-xs text-muted-foreground truncate">
                  {[r.other?.city, r.other?.state, r.other?.sub_sect].filter(Boolean).join(" · ")}
                </div>
              </div>
              ...
              {r.status === "accepted" && r.other && (
                <Button size="sm" onClick={() => openChat(r.other!.id)}>
