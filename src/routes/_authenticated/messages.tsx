...
function MessagesList() {
  ...
  const [conversations, setConversations] = useState<
    Array<Conv & { other: ProfileLite | null; preview: string | null }>
  >([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      setConversations(
        list.map((c, i) => ({
          ...c,
          other: byId.get(c.profile_a === u.user!.id ? c.profile_b : c.profile_a) ?? null,
          preview: previews[i],
        })),
      );
    })();
        ...
        {me === null ? null : conversations.length === 0 ? (
          ...
          conversations.map((c) => (
            <Link
              ...
              to="/messages/$id"
              params={{ id: c.id }}
              className="block p-4 rounded-2xl bg-card border border-border hover:shadow-sm transition"
            >
