        ...
        {step === 0 && (
          <>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Profile is for">
                <Sel
                  value={f.on_behalf_of}
                  onChange={(v) => set("on_behalf_of", v)}
                  options={[
                    ["self", "Self"],
                    ["son", "My son"],
        ...
        {step === 1 && (
          <>
            <Field label="Sub-sect">
              <Sel
                value={f.sub_sect}
                onChange={(v) => set("sub_sect", v)}
                options={SUB_SECTS.map((s) => [s, s])}
              />
            </Field>
            <div className="grid grid-cols-2 gap-4">
              <Field label="Gotra">
                <Input value={f.gotra} onChange={(e) => set("gotra", e.target.value)} />
              </Field>
              <Field label="Guru lineage / Ayya">
                <Input
                  value={f.guru_lineage}
                  onChange={(e) => set("guru_lineage", e.target.value)}
                />
              </Field>
            </div>
          ...
          <>
            <Field label="Education">
              <Input
                ...
                placeholder="B.E. Computer Science, IIT Bombay"
              />
            </Field>
            ...
            <Field label="Annual income (INR)">
              <Input
                type="number"
                value={f.annual_income_inr}
                onChange={(e) => set("annual_income_inr", e.target.value)}
              />
            </Field>
            <Field label="Diet">
              <Sel
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Native district">
                <Input
                  value={f.native_district}
                  onChange={(e) => set("native_district", e.target.value)}
                  placeholder="Bijapur, Belgaum…"
                />
              </Field>
            </div>
          </>
          ...
          <>
            <Field label="About yourself">
              <Textarea
                ...
                placeholder="Share your values, hobbies, and what you're looking for."
              />
            </Field>
          </>
        )}
        <div className="flex items-center justify-between pt-2">
          ...
          {step < total - 1 ? (
            ...
            <Button onClick={() => save(true)} disabled={saving}>
              {saving ? "Activating…" : "Finish & activate profile"}
            </Button>
          )}
        </div>
      </div>
