      ...
      <div className="flex items-center gap-3 pb-4 border-b border-border">
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-muted-foreground">
            End-to-end privacy · verified members only
          </div>
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto py-6 space-y-3">
        {messages.length === 0 ? (
          <div className="text-center text-sm text-muted-foreground py-12">
            No messages yet. Send a warm hello 👋
          </div>
        ) : (
          messages.map((m) => {
            ...
            return (
              <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[75%] rounded-2xl px-4 py-2 text-sm leading-relaxed ${mine ? "bg-primary text-primary-foreground rounded-br-sm" : "bg-muted text-foreground rounded-bl-sm"}`}
                >
                  {m.body}
                  <div
                    className={`text-[10px] mt-1 opacity-70 ${mine ? "text-primary-foreground" : "text-muted-foreground"}`}
                  >
