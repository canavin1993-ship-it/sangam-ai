...
type Row = { id: string; profile_id: string; note: string | null; profile: { id: string; display_name: string; city: string | null; state: string | null; sub_sect: string | null } | null };
...
function ShortlistPage() {
  ...
  const load = async () => {
    ...
    const { data } = await supabase
      .from("shortlists")
      .select("id, profile_id, note, profile:profiles!shortlists_profile_id_fkey(id, display_name, city, state, sub_sect)")
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-8 pb-24">
      ...
      <p className="text-sm text-muted-foreground mb-6">Save profiles here to review with family.</p>
      ...
      {rows.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 gap-4 mt-2">
          {rows.map((r) => (
            <div key={r.id} className="p-5 rounded-2xl bg-card border border-border flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <Link to="/profile/$id" params={{ id: r.profile?.id ?? "" }} className="font-medium hover:underline">{r.profile?.display_name ?? "Member"}</Link>
                <div className="text-xs text-muted-foreground mt-0.5">{[r.profile?.city, r.profile?.state, r.profile?.sub_sect].filter(Boolean).join(" · ")}</div>
