...
function AuthPage() {
  ...
  const onGoogle = async () => {
    ...
    if (result.error) { setLoading(false); return toast.error(String(result.error.message ?? result.error)); }
    ...
    <div className="min-h-screen grid md:grid-cols-2 bg-background">
      <div className="hidden md:flex flex-col justify-between p-12 bg-primary text-primary-foreground relative overflow-hidden">
        <Link to="/" className="flex items-center gap-2 relative z-10">
          <span className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-accent text-accent-foreground font-display text-lg">ಜ</span>
          <span className="font-display text-xl">Jangama Matrimony</span>
        </Link>
        <div className="relative z-10">
          <h2 className="font-display text-4xl leading-tight">Meaningful matches, built on trust.</h2>
          <p className="mt-4 text-primary-foreground/80">Verified profiles. AI compatibility. A family-first workflow made for the Jangama community.</p>
        </div>
        <div className="text-xs text-primary-foreground/60 relative z-10">Featured in the Jangama Sangha newsletter</div>
        <div className="absolute inset-0 opacity-20" style={{ backgroundImage: "radial-gradient(circle at 20% 30%, var(--accent), transparent 40%), radial-gradient(circle at 80% 70%, var(--gold), transparent 45%)" }} />
      </div>
      <div className="flex items-center justify-center p-6 md:p-12">
        <div className="w-full max-w-md">
          <div className="md:hidden mb-8">
            <Link to="/" className="font-display text-2xl text-primary">Jangama Matrimony</Link>
          </div>
          <Tabs defaultValue="signin">
            ...
            <TabsContent value="signin" className="mt-6">
              <form onSubmit={onSignIn} className="space-y-4">
                ...
                <Button type="submit" className="w-full h-11" disabled={loading}>{loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Sign in"}</Button>
              </form>
            </TabsContent>
            <TabsContent value="signup" className="mt-6">
              <form onSubmit={onSignUp} className="space-y-4">
                ...
                <Button type="submit" className="w-full h-11" disabled={loading}>{loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Create free account"}</Button>
              </form>
            </TabsContent>
          </Tabs>
          ...
          <Button variant="outline" className="mt-6 w-full h-11" onClick={onGoogle} disabled={loading}>
